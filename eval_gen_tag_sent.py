...
def tagging(index, span, tags):
    for i in range(len(span)):
        if i == 0:
            ...
            tags[index[0] + i] = 'I'
    return tags
def tagging_sent(index, span, sent, tags):
    for i in range(len(span)):
        if i == 0:
            tags[index[0] + i] = f'B-{sent}'
        else:
            tags[index[0] + i] = f'I-{sent}'
    return tags
import pandas as pd
import ast
...
for file in files:
    ...
    for (text, labels, predicts) in zip(df.text.values, df.label.values, df.prediction.values):
        ...
        for label in labels.split('|'):
            ...
            if len(label_idx) > 0:
                label_tags = tagging_sent(label_idx, label, sent, label_tags)
        for prediction in predicts.split('|'):
            ...
            if len(predict_idx) > 0:
                predict_tags = tagging_sent(predict_idx, prediction, sent, predict_tags)
...
files = ['output/ABSA16FR_Restaurants_Gold-withcontent_xlm-roberta-large_with_sentiment..csv',   'output/EN_REST_SB1_TEST_xlm-roberta-large_with_sentiment..csv',
'output/DU_REST_SB1_TEST_xlm-roberta-large_with_sentiment..csv','output/SP_REST_SB1_TEST_xlm-roberta-large_with_sentiment..csv'] 
...
for file in files:
    ...
    for (text, labels, predicts) in zip(df.text.values, df.label.values, df.prediction.values):
        ...
        for label in labels:
            sent = label[1]
            label = label[0].strip().split()
            label_idx = [idx for idx in range(len(text) - len(label) + 1) if text[idx : idx + len(label)] == label]
            if len(label_idx) > 0:
                label_tags = tagging_sent(label_idx, label, sent, label_tags)
        for prediction in predicts:
            sent = prediction[1]
            prediction = prediction[0].strip().split()
            predict_idx = [idx for idx in range(len(text) - len(prediction) + 1) if text[idx : idx + len(prediction)] == prediction]
            if len(predict_idx) > 0:
                predict_tags = tagging_sent(predict_idx, prediction, sent, predict_tags)
        all_label_tags.append(label_tags)
